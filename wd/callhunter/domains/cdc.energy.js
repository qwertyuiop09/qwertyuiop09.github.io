...
var SSCHunterCallback = function(type,undefined,undefined,undefined,undefined,undefined,Conv_value) { if (type == 200) { console.log ( 'ConvTime exist' ); if (typeof window.ga == 'function') { ga('send', 'event', 'Hot Activity', 'ss_hot_activity'); ga('CallHunterTracker.send', 'event', 'Hot Activity', 'ss_hot_activity'); } } else if ((type >= 1)&&(type < 200)) { console.log ( 'ConvCHSnd exist' ); if (typeof window.ga == 'function') { ga('send', 'event', 'Client Hunter', 'ss_send_message'); ga('CallHunterTracker.send', 'event', 'Client Hunter', 'ss_send_message'); } } else if (type == 201) { console.log ( 'ConvСlick exist' ) } else if (type == 202) { console.log ( 'ConvSubmit exist' ) } else if (type == 203) { console.log ( 'ConvHit exist' ); if (typeof window.ga == 'function') { ga('send', 'event', 'View Page', 'ss_page_hit'); ga('CallHunterTracker.send', 'event', 'View Page', 'ss_page_hit'); } } else if (type == 204) { console.log ( 'ConvJivo exist' ) } else if (type == 205) { console.log ( 'CallWidget exist' ) } ; if (typeof Conv_value != 'undefined') { console.log ( 'ConvValue exist: ' + Conv_value ) }   }
...
//		"PRIFormDataPh": {title:'Акция для входящих звонков', titleworktime:'Спасибо, подарок уже ждет Вас!', titlenight:'Спасибо, подарок уже ждет Вас!', titlenightsize: "12px", fields: "ХОЧУ СПЕЦАКЦИЮ(Ваше имя и телефон для участия в акции), <Одноразовый ID(узнайте ID у менеджера)*", button: "Отправить", callTime: 0},
...
//	"workDays": "Mon, Tue, Wed, Thu, Fri",
	...
	google_remarketing_only: false
		...
		"MMFileType": "2",
		"ExtFormFCStop": {RegExp: '<[^>]+(search|subscribe|password|calculator|Расчитать)', Flags: 'i'},
