		...
		if (window.intepriceCRT) {
			...
			if (match) {
				...
				if ((currd.getTime() - sttime.getTime())/(24*60*60*1000) < 30) {
					...
					currd.setTime(currd.getTime() + (currd.getTimezoneOffset()*60*1000) + ((IPclhrDataUpdate["timeZone"] ? IPclhrDataUpdate["timeZone"] : IPclhrData["timeZone"])*60*60*1000) + Number(window.IPcallhunterDiff));
